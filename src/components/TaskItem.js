...
export default function TaskItem({ task, onPress, onSwipeComplete, onSwipeDelete, onToggleStep, onToggleStar }) {
  ...
  function getCardStyle() {
    ...
    if (cardStyle === 'bordered') {
      return { borderRadius: radius, borderWidth: 1, borderColor: theme.separator, ...shadow };
    }
    if (cardStyle === 'flat') {
  ...
  const renderRightActions = () => {
    let actionText = '完成';
    let actionIcon = '✓';
    if (hasSteps && !allStepsCompleted) { actionText = `下一步 (${completedSteps}/${totalSteps})`; actionIcon = '▶'; }
    return (
  ...
  const onSwipeableOpen = (direction) => {
    if (direction === 'left') onSwipeDelete();
    else if (direction === 'right') onSwipeComplete();
  ...
  return (
    <Swipeable ref={swipeableRef} renderLeftActions={renderLeftActions} renderRightActions={renderRightActions} onSwipeableOpen={onSwipeableOpen} leftThreshold={SWIPE_THRESHOLD} rightThreshold={SWIPE_THRESHOLD} friction={1.5} overshootFriction={8}>
      <TouchableOpacity style={[styles.card, { backgroundColor: getCardBackground() }, getCardStyle(), { opacity: isDone ? 0.6 : 1 }]} onPress={() => onPress(task)} onLongPress={() => onPress(task)} activeOpacity={0.8} delayLongPress={400}>
        {/* 左侧彩色细条 */}
          ...
          <View style={styles.metaRow}>
            ...
            {getRecurrenceLabel() ? (
              <View style={[styles.badge, { backgroundColor: bgColor }]}>
                <ThemedText style={[styles.badgeText, { color: labelColor }]}>↻ {getRecurrenceLabel()}</ThemedText>
              </View>
            ) : null}
            {hasSteps && (
              <View style={[styles.badge, { backgroundColor: bgColor }]}>
                <ThemedText style={[styles.badgeText, { color: taskColor }]}>✓ {completedSteps}/{totalSteps}</ThemedText>
              </View>
            )}
            {getStatusLabel() ? (
              <View style={[styles.badge, { backgroundColor: (isOverdue ? theme.danger : taskColor) + '20' }]}>
                <ThemedText style={[styles.badgeText, { color: isOverdue ? theme.danger : taskColor }]}>{getStatusLabel()}</ThemedText>
              </View>
            ) : null}
          </View>
        </View>
        ...
        <View style={styles.rightIndicator}>
          {onToggleStar && (
            <TouchableOpacity onPress={() => onToggleStar(task.id)} style={styles.starButton}>
              <Text style={[styles.starText, { color: task.is_starred ? '#F59E0B' : theme.textTertiary }]}>{task.is_starred ? '★' : '☆'}</Text>
            </TouchableOpacity>
          )}
...
function createStyles(theme) {
  return StyleSheet.create({
    card: { flexDirection: 'row', marginHorizontal: 16, marginVertical: 4, overflow: 'hidden' },
    ...
    actionContainer: { flex: 1, justifyContent: 'center', marginVertical: 4 },
    actionInner: { flexDirection: 'column', alignItems: 'center', justifyContent: 'center', paddingHorizontal: 24, gap: 4 },
