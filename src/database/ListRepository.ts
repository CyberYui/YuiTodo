/**
 * Task list repository — CRUD for task lists.
 */
...
export async function createList(name: string, icon = '📋', sortOrder = 0): Promise<number> {
  ...
  return getRows<{ id: number }>(result[0])[0]?.id || 0;
}

export async function getAllLists(): Promise<TaskList[]> {
  const db = getDatabase();
  const result = await db.execAsync(
    [{ sql: 'SELECT * FROM task_list ORDER BY sort_order ASC', args: [] }],
    true
  );
  return getRows<TaskList>(result[0]);
}

export async function updateList(listId: number, updates: Partial<TaskList>): Promise<void> {
  const db = getDatabase();
  const now = Date.now();
